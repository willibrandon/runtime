...
public static unsafe partial class ProbeExports
{
        ...
        delegate* unmanaged[Cdecl]<int, long, void> report)
    {
        try
        {
            return stage switch
            {
                ...
                3 => CheckCollection(nativePid, parentPid, tokenLow, tokenHigh, report),
                4 => CheckThreadPool(report),
                5 => CheckTimer(report),
                6 => CheckException(report),
                7 => InitializeActiveHeap(report),
    ...
    /// <param name="mutateChild">Whether to perform the independent child mutation after validating inheritance.</param>
    ...
    private static int CheckGraph(int nativePid, int parentPid, long tokenLow, long tokenHigh,
        delegate* unmanaged[Cdecl]<int, long, void> report, bool mutateChild = true)
    {
        ...
        if (mutateChild && nativePid != parentPid)
        {
            root.Marker = 731;
            s_numbers[0] = -73;
    ...
    /// </summary>
    /// <param name="nativePid">The independent current native process identity.</param>
    /// <param name="parentPid">The independently retained initializer process identity.</param>
    /// <param name="tokenLow">The original token's first native word.</param>
    /// <param name="tokenHigh">The original token's second native word.</param>
        ...
        delegate* unmanaged[Cdecl]<int, long, void> report)
    {
        ...
        report(35, background.Concurrent ? 1 : 0);
        if (background.Index <= previousBackgroundIndex || !background.Concurrent || background.Generation != GC.MaxGeneration)
        {
            return 33;
        }
        ...
        for (int index = 0; index < retained.Length; index++)
        {
            if (retained[index].Length != 16384 || retained[index][0] != (byte)index || retained[index][^1] != 0)
            {
                return 34;
            }
        }
        ...
        return finalized && completion.Value == 73 && !unreachable.IsAlive
            ? CheckGraph(nativePid, parentPid, tokenLow, tokenHigh, report) : 30;
    }
